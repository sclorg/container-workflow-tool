...
class CliCommon(object):
    ...
    def get_parser(self):
        ...
        parser.add_argument(
            "--tmp", help="Overrides default temporary working directory"
        )
        parser.add_argument(
            "--clear-cache",
            action="store_true",
        ...
        parsers["git"].add_argument(
            "--rebuild-reason", help="Use a custom reason for rebuilding"
        )
        parsers["git"].add_argument(
            "--commit-msg", help="Use a custom message instead of the default one"
        )
        parsers["git"].add_argument(
            "--check-script", help="Script/command to be run when checking repositories"
        )
        return parser
